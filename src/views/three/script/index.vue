      ...
      <el-table-column type="expand" width="50%">
        <template slot-scope="scope">
          <el-table v-loading="scope.row.loadingChild" :row-key="scope.row.child.id" :data="scope.row.child"
            ...
            <el-table-column label="支付通道名称" align="center" prop="payWayName"/>
            <el-table-column label="支付通道编码" align="center" prop="payWayCode"/>
            ...
            <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
              <template slot-scope="scope">
                ...
                <el-button
                  ...
                  v-hasPermi="['three:script:remove']"
                >删除
                </el-button>
              </template>
            </el-table-column>
  ...
  methods: {
    ...
    expandChange(row, expandedRows) {
      ...
      if (expandedRows.length) {
        ...
        listScript(this.queryParamsParent).then(response => {
      ...
      this.$refs["formChild"].validate(valid => {
        if (valid) {
          if (this.formChild.id != null) {
            updateScript(this.formChild).then(response => {
              this.msgSuccess("脚本修改成功");
              this.openChild = false;
              this.getList();
            });
          } else {
            addScript(this.formChild).then(response => {
